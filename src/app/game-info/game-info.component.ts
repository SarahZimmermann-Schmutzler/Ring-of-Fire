...
export class GameInfoComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.card) {
      ...
      console.log('Current number is', this.card.split('_'));
      console.log('Current number is', this.card.split('_')[1]);
      let cardNumber = +this.card.split('_')[1];
      this.title = this.cardAction[cardNumber - 1].title;
      //this title bezieht sich auf die angelegte Variable, an diese wird gebunden
      //der titel des arrays cardAction an der Stelle cardNumber
      // -1 weil Karten von 1-13 gehen, aber Array (cardAction) von 0-12
      this.description = this.cardAction[cardNumber - 1].description;
    }
    // funktion ausführen, wenn card bereits einen wert hat
  }
}
